...
let generate_a_ena_node_dict = function (
  ...
  let calculated_node_size = calculating_weights(
    node_frequency,
    0,
    250,
    5,
...
let generate_node_data = function (node_name, node_size) {
  if (["handover"].includes(node_name)) {
    return generate_a_ena_node_dict(
    ...
    for (let other_code in raw_json_data[current_code]) {
      ...
      edge_width.push({
        ...
        data: {
          id:
            formating_node_id(
              NODE_NAME_MAPPER[current_code],
              node_size_map[current_code]
            ) +
            "_" +
            formating_node_id(
              NODE_NAME_MAPPER[other_code],
              node_size_map[other_code]
            ),
          source: formating_node_id(
            NODE_NAME_MAPPER[current_code],
            node_size_map[current_code]
          ),
          target: formating_node_id(
            NODE_NAME_MAPPER[other_code],
            node_size_map[other_code]
          ),
        },
