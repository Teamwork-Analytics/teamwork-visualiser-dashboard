...
let regenerating_df = function (a_df_slice) {
...
let processing_csv = function (
  ...
  if (raw_json_data.length === 0) return {};

  let test_df = new dfd.DataFrame(raw_json_data);

  if (use_comm) {
  }
  // moved into for loop, because glitch will happen if use mask loading on slice

  let df_slice = test_df.iloc({ rows: test_df["start_time"].gt(start_time) });
  df_slice = df_slice.iloc({ rows: test_df["start_time"].lt(end_time) });
  // df_slice["initiator"].print();
  df_slice = regenerating_df(df_slice);
