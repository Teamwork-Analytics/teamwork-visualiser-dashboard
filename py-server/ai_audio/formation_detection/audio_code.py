...
def extract_vad(audio_folder_path: str):
    processed_audio_folder_path = os.path.join(audio_folder_path, "audio", "processed_audio")
    res_dict = {}
    for a_filename in os.listdir(processed_audio_folder_path):
        if ".wav" not in a_filename:
            print("{} is not a audio file, skipped".format(os.path.join(processed_audio_folder_path, a_filename)))
            continue
        if "sim_" not in a_filename:
            print("{} is not a PROCESSED audio file, skipped".format(os.path.join(processed_audio_folder_path, a_filename)))
            continue
        print("start VAD on {}".format(os.path.join(processed_audio_folder_path, a_filename)))
        path = os.path.join(processed_audio_folder_path, a_filename)
